...
| **Relatórios de Execução (físico)**                               | Informações sobre resgates e atendimentos: contexto, exames realizados, gastos e adoções do período. | Digitação manual via AppSheet, registrando cada operação e seus detalhes.                                                                                                   | Tabelas `resgates`, `atendimentos_clinicos`, `despesas`, `adocoes` |
